import * as van from "./van-0.11.0.js" 
...
class State extends van.state().constructor {
  constructor(v) {
...
export const state = initVal => new State(initVal)
...
const checkChildValid = child => {
  expect(
    isDomOrPrimitive(child) || child instanceof State && isValidPrimitive(child.val),
    ...
    return (...args) => {
      ...
      for (const [k, v] of Object.entries(props)) {
        ...
        if (v instanceof State) {
          debugProps[k] = {deps: [v], f: v => (validatePropValue(v), v)}
        } else if (v?.constructor === Object) {
          expect(Array.isArray(v.deps) && v.deps.every(d => d instanceof State),
