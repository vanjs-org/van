(() => {
  ...
  var changedStates;
  var enqueueState = (s) => changedStates = (changedStates ?? (setTimeout(updateDoms, 0), /* @__PURE__ */ new Set())).add(s);
  var setState = (s, val) => {
    if (val !== s.val) {
      if (!s.oldVal) {
        enqueueState(s);
  ...
  var updateDoms = () => {
    ...
    changedStates = Null;
    new Set(changedStatesArray.map((s) => s.bindings = s.bindings.filter((b) => b.dom.isConnected)).flat()).forEach((b) => {
