...
let bindingQueue = new Set()

let enqueueBinding = binding => {
  let isFirst = bindingQueue.size === 0
  bindingQueue.add(binding)
  if (isFirst) setTimeout(dequeueBindingAndUpdateDom, 0)
}

let dequeueBindingAndUpdateDom = () => {
  let allBindings = bindingQueue
  bindingQueue = new Set()
...
export let setState = (states, vals) => {
  ...
  for (let i = 0; i < states.length; ++i) {
    ...
    if (vals[i] !== s.val) {
      s.oldVal = s.val
      s.val = vals[i]
      for (let b of s.bindings) enqueueBinding(b)
    }
  }
}
