(() => {
  ...
  var stateProto2 = van_default.state().__proto__;
  var checkStateValValid = (v) => {
    expect(!(v instanceof Node), "DOM Node is not valid value for state");
    expect(v?.__proto__ !== stateProto2, "State couldn't have value to other state");
    return v;
  };
  ...
  var checkChildValid = (child) => {
    expect(
      isDomOrPrimitive(child) || child?.__proto__ === stateProto2 && isValidPrimitive(child.val),
  ...
  var tags2 = new Proxy(van_default.tags, {
    get: (vanTags, name) => {
      ...
      return (...args) => {
        const [props, ...children] = args[0]?.__proto__ === Object.prototype ? args : [{}, ...args];
        const debugProps = {};
        for (const [k, v] of Object.entries(props)) {
          ...
          if (v?.__proto__ === stateProto2) {
            debugProps[k] = { deps: [v], f: (v2) => validatePropValue(v2) };
          } else if (v?.__proto__ === Object.prototype) {
            expect(
              Array.isArray(v.deps) && v.deps.every((d) => d.__proto__ === stateProto2),
  ...
  var bind2 = (...args) => {
    ...
    deps.forEach((d) => expect(d.__proto__ === stateProto2, "Dependencies in `bind` must be states"));
