import * as van from "./van-0.9.3.js" 
...
export const bind = (...args) => {
  ...
  expect(result !== null && result !== undefined, "The initial result of `bind` generation function can't be null or undefined")
  ...
  return van.bind(...deps, (...genArgs) => {
    ...
    if (genArgs.length > deps.length) {
      ...
      if (prevResult instanceof Node) {
        ...
        if (!expect(result instanceof Node || !result,
        ...
        if (result && result !== prevResult)
          if (!expect(!result.isConnected,
            "For binding to DOM Node, if the result of generation function is not the same as previous one, it shouldn't be already connected to a DOM document")) return null
      } else {
