...
const runTests = async (van, msgDom, { debug }) => {
    const { a, button, div, input, li, option, p, pre, select, span, table, tbody, td, th, thead, tr, ul } = van.tags;
    const assert = (cond) => {
        if (!cond)
    ...
    const tests = {
        ...
        tags_stateDerivedOnClickHandler_connected: withHiddenDom(async (hiddenDom) => {
            const dom = div();
            van.add(hiddenDom, dom);
            const addPElement = van.state(true);
            van.add(dom, button({ onclick: { deps: [addPElement], f: addPElement => addPElement ?
                        () => van.add(dom, p("Button clicked!")) :
                        () => van.add(dom, div("Button clicked!"))
                } }));
            dom.querySelector("button").click();
            assertEq(dom.outerHTML, "<div><button></button><p>Button clicked!</p></div>");
        ...
        bind_dynamicDom: withHiddenDom(async (hiddenDom) => {
            ...
            const dom = van.bind(verticalPlacement, v => v ? div(div(button(button1Text)), div(button(button2Text)), div(button(button3Text))) : div(button(button1Text), button(button2Text), button(button3Text)));
            assertEq(van.add(hiddenDom, dom), hiddenDom);
        ...
        bind_statefulDynamicDom: withHiddenDom(async (hiddenDom) => {
            ...
            const selectedIndex = van.state(0);
            items.onnew(() => selectedIndex.val = 0);
            const dom = van.bind(items, selectedIndex, (items, selectedIndex, dom, oldItems, oldSelectedIndex) => {
        ...
        bind_nullToRemoveDom: withHiddenDom(async (hiddenDom) => {
            ...
            const dom = div(van.bind(line1, l => l === "" ? null : p(l)), van.bind(line2, l => l === "" ? null : p(l)), p(line3), 
            // line4 won't appear in the DOM tree as its initial value is null
            van.bind(line4, l => l === "" ? null : p(l)), 
        ...
        bind_undefinedToRemoveDom: withHiddenDom(async (hiddenDom) => {
            ...
            const dom = div(van.bind(line1, l => l === "" ? undefined : p(l)), van.bind(line2, l => l === "" ? undefined : p(l)), p(line3), 
            // line4 won't appear in the DOM tree as its initial value is undefined
            van.bind(line4, l => l === "" ? undefined : p(l)), 
        ...
        bind_nonStateDeps: withHiddenDom(async (hiddenDom) => {
            ...
            const dom = van.bind(part1, part2, (part1, part2) => part1 + part2);
            assertEq(van.add(hiddenDom, dom), hiddenDom);
    ...
    const debugTests = {
        ...
        tags_invalidProp_nonPrimitiveValue: () => {
            ...
            // State as property
            assertError(/Only.*are valid prop value types/, () => a({ href: van.state({}) }));
            assertError(/Only.*are valid prop value types/, () => a({ href: van.state(null) }));
            assertError(/Only.*are valid prop value types/, () => a({ href: van.state(undefined) }));
            assertError(/Only.*are valid prop value types/, () => a({ href: van.state((x) => x * 2) }));
        ...
        tags_invalidChild: () => {
            ...
            assertError(/Only.*are valid child of a DOM Node/, () => div(div(), van.state({}), p()));
            assertError(/Only.*are valid child of a DOM Node/, () => div(div(), van.state(((x) => x * 2)), p()));
        },
        ...
        add_1stArgNotDom: () => {
            assertError("1st argument of `add` function must be a DOM Node object", () => van.add({}, div()));
        },
        add_invalidChild: () => {
            const dom = div();
            assertError(/Only.*are valid child of a DOM Node/, () => van.add(dom, div(), {}, p()));
            assertError(/Only.*are valid child of a DOM Node/, () => van.add(dom, div(), ((x) => x * 2), p()));
            assertError(/Only.*are valid child of a DOM Node/, () => van.add(dom, div(), van.state({}), p()));
            assertError(/Only.*are valid child of a DOM Node/, () => van.add(dom, div(), van.state(((x) => x * 2)), p()));
        },
        ...
        state_invalidInitialVal: () => {
            assertError("DOM Node is not valid", () => van.state(document));
        ...
        bind_noStates: () => {
            // @ts-ignore
            assertError("1 or more states", () => van.bind());
            // @ts-ignore
            assertError("1 or more states", () => van.bind(x => x * 2));
        },
        bind_lastArgNotFunc: () => assertError("must be the generation function", () => van.bind(van.state(0), van.state(1))),
        bind_invalidInitialResult: () => {
            const s = van.state(0);
            assertError("must be DOM node, primitive, null or undefined", () => van.bind(s, (() => ({}))));
            assertError("must be DOM node, primitive, null or undefined", () => van.bind(s, (() => x => x * 2)));
        },
        bind_invalidFollowupResult: withHiddenDom(async (hiddenDom) => {
            const s = van.state(1);
            van.add(hiddenDom, van.bind(s, (s => s || {})), van.bind(s, (s => s || (x => x * 2))));
            await capturingErrors(async () => {
                s.val = 0;
                await sleep(waitMsOnDomUpdates);
                assert(van.capturedErrors.length === 2 &&
                    van.capturedErrors.every(e => e.includes("must be DOM node, primitive, null or undefined")));
            });
        }),
        bind_derivedDom_domResultAlreadyConnected: withHiddenDom(async (hiddenDom) => {
            ...
            van.add(hiddenDom, van.bind(num, (num, prevDom) => {
                if (num === 1)
                    return div();
                if (num === 2)
                    return prevDom;
            ...
            await capturingErrors(async () => {
                num.val = 3;
                await sleep(waitMsOnDomUpdates);
                assert(van.capturedErrors[0].includes("it shouldn't be already connected to document"));
            });
        }),
    };
    ...
    const examples = {
        ...
        stateExample: withHiddenDom(async (hiddenDom) => {
            ...
            const dom4 = div(van.bind(counter, c => `${c}^2 = ${c * c}`));
        ...
        sortedList: withHiddenDom(async (hiddenDom) => {
            const SortedList = () => {
                ...
                return span("Comma-separated list: ", input({ oninput: e => items.val = e.target.value,
                    type: "text", value: items }), " ", select({ oninput: e => sortedBy.val = e.target.value, value: sortedBy }, option({ value: "Ascending" }, "Ascending"), option({ value: "Descending" }, "Descending")), van.bind(items, sortedBy, (items, sortedBy) => sortedBy === "Ascending" ?
        ...
        editableList: withHiddenDom(async (hiddenDom) => {
            const ListItem = ({ text }) => {
                ...
                return van.bind(deleted, d => d ? null : li(text, a({ onclick: () => deleted.val = true }, "❌")));
            };
            const EditableList = () => {
                const listDom = ul();
    ...
    const gcTests = {
        derivedDom: withHiddenDom(async (hiddenDom) => {
            ...
            const text = van.state("Text");
            const TextLine = (renderPre) => (renderPre ? pre : div)(van.bind(text, t => `--${t}--`));
