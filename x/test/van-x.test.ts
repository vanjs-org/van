...
window.runTests = async (van: Van, vanX: typeof vanXObj, file: string) => {
  ...
  const tests = {
    ...
    noreactive_obj: withHiddenDom(async hiddenDom => {
      const data = vanX.reactive(<Record<string, ArrayBuffer>>{
    ...
    raw_array: withHiddenDom(async hiddenDom => {
      const printToStr = (arr: number[]) => {
        let result = ""
        for (let i = 0; i < arr.length; ++i) result += `${i ? ", " : ""}${i}: ${arr[i]}`
        return result
      }

      const render = (arr: number[]) => div(
        div("Length is: ", () => arr.length),
        div("Elements are: ", () => printToStr(arr)),
        div("Mapped elements are: ", () => printToStr(arr.map(t => t * 2)))
      )
